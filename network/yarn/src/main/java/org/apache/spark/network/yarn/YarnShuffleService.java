...
public class YarnShuffleService extends AuxiliaryService {
  ...
  private File findRegisteredExecutorFile(String[] localDirs) {
    for (String dir: localDirs) {
      File f = new File(new Path(dir).toUri().getPath().toString(), "registeredExecutors.ldb");
      if (f.exists()) {
        return f;
      }
    }
    return new File(new Path(localDirs[0]).toUri().getPath().toString(), "registeredExecutors.ldb");
  }
